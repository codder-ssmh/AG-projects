		...
		$pcConfig = array(
			...
			'incomes' => array(   
				'client' => array(   
					...
					'child-primary-key' => 'income_id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Incomes <span class="hidden child-label-incomes child-field-caption">(Client)</span>',
					'auto-close' => false,
					'table-icon' => 'resources/table_icons/coins_add.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Date', 2 => 'Client', 3 => 'Amount'),
					'display-field-names' => array(1 => 'date', 2 => 'client', 3 => 'amount'),
					'sortable-fields' => array(0 => '`incomes`.`income_id`', 1 => '`incomes`.`date`', 2 => '`clients1`.`client`', 3 => '`incomes`.`amount`'),
					'records-per-page' => 10,
					'default-sort-by' => 1,
					...
					'template' => 'children-incomes',
					'template-printable' => 'children-incomes-printable',
					'query' => "SELECT `incomes`.`income_id` as 'income_id', if(`incomes`.`date`,date_format(`incomes`.`date`,'%m/%d/%Y'),'') as 'date', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `incomes`.`amount` as 'amount' FROM `incomes` LEFT JOIN `clients` as clients1 ON `clients1`.`client_id`=`incomes`.`client` "
				)
			),
			'expenses' => array(   
				'client' => array(   
					'parent-table' => 'clients',
					'parent-primary-key' => 'client_id',
					'child-primary-key' => 'expense_id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Expenses <span class="hidden child-label-expenses child-field-caption">(Client)</span>',
					'auto-close' => false,
					'table-icon' => 'resources/table_icons/coins_delete.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Date', 2 => 'Client', 3 => 'Amount'),
					'display-field-names' => array(1 => 'date', 2 => 'client', 3 => 'amount'),
					'sortable-fields' => array(0 => '`expenses`.`expense_id`', 1 => '`expenses`.`date`', 2 => '`clients1`.`client`', 3 => '`expenses`.`amount`'),
					'records-per-page' => 10,
					'default-sort-by' => 1,
					'default-sort-direction' => 'desc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-expenses',
					'template-printable' => 'children-expenses-printable',
					'query' => "SELECT `expenses`.`expense_id` as 'expense_id', if(`expenses`.`date`,date_format(`expenses`.`date`,'%m/%d/%Y'),'') as 'date', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `expenses`.`amount` as 'amount' FROM `expenses` LEFT JOIN `clients` as clients1 ON `clients1`.`client_id`=`expenses`.`client` "
				)
			)
		);
