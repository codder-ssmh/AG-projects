	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'clients' => array('Available budget', '', 'resources/table_icons/folder_user.png', 'None'),
			'incomes' => array('Incomes', '', 'resources/table_icons/coins_add.png', 'None'),
			'expenses' => array('Expenses', '', 'resources/table_icons/coins_delete.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'clients' => "`clients`.`client_id` as 'client_id', `clients`.`client` as 'client', `clients`.`total_income` as 'total_income', `clients`.`total_expense` as 'total_expense', `clients`.`available_budget` as 'available_budget'",
			'incomes' => "`incomes`.`income_id` as 'income_id', if(`incomes`.`date`,date_format(`incomes`.`date`,'%m/%d/%Y'),'') as 'date', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `incomes`.`amount` as 'amount'",
			'expenses' => "`expenses`.`expense_id` as 'expense_id', if(`expenses`.`date`,date_format(`expenses`.`date`,'%m/%d/%Y'),'') as 'date', IF(    CHAR_LENGTH(`clients1`.`client`), CONCAT_WS('',   `clients1`.`client`), '') as 'client', `expenses`.`amount` as 'amount'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(   
			'clients' => "`clients` ",
			'incomes' => "`incomes` LEFT JOIN `clients` as clients1 ON `clients1`.`client_id`=`incomes`.`client` ",
			'expenses' => "`expenses` LEFT JOIN `clients` as clients1 ON `clients1`.`client_id`=`expenses`.`client` "
		);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> budgety</a>
			</div>
