...
function expenses_insert(){
	...
	sql('insert into `expenses` set       `date`=' . (($data['date'] !== '' && $data['date'] !== NULL) ? "'{$data['date']}'" : 'NULL') . ', `client`=' . (($data['client'] !== '' && $data['client'] !== NULL) ? "'{$data['client']}'" : 'NULL') . ', `amount`=' . (($data['amount'] !== '' && $data['amount'] !== NULL) ? "'{$data['amount']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
	...
	if(function_exists('expenses_after_insert')){
		$res = sql("select * from `expenses` where `expense_id`='" . makeSafe($recID, false) . "' limit 1", $eo);
	...
	set_record_owner('expenses', $recID, getLoggedMemberID());
...
function expenses_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	sql("delete from `expenses` where `expense_id`='$selected_id'", $eo);
...
function expenses_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `expenses` set       `date`=' . (($data['date'] !== '' && $data['date'] !== NULL) ? "'{$data['date']}'" : 'NULL') . ', `client`=' . (($data['client'] !== '' && $data['client'] !== NULL) ? "'{$data['client']}'" : 'NULL') . ', `amount`=' . (($data['amount'] !== '' && $data['amount'] !== NULL) ? "'{$data['amount']}'" : 'NULL') . " where `expense_id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
	...
	if(function_exists('expenses_after_update')){
		$res = sql("SELECT * FROM `expenses` WHERE `expense_id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['expense_id'];
		$args = array();
...
function expenses_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `expenses` where `expense_id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'expenses_view.php', false);
		}
			...
			$j("#client-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_client__RAND__.value, t: 'expenses', f: 'client' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'expenses', f: 'client' }; },
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/expenses_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/expenses_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Expense details', $templateCode);
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return expenses_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(expense_id)%%>', '', $templateCode);
	...
	// process values
	if($selected_id){
		if( $dvprint) $templateCode = str_replace('<%%VALUE(expense_id)%%>', safe_html($urow['expense_id']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(expense_id)%%>', html_attr($row['expense_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(expense_id)%%>', urlencode($urow['expense_id']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(expense_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(expense_id)%%>', urlencode(''), $templateCode);
	...
	$rdata = $jdata = get_defaults('expenses');
	if($selected_id){
		$jdata = get_joined_record('expenses', $selected_id);
		if($jdata === false) $jdata = get_defaults('expenses');
		$rdata = $row;
	}
	$templateCode .= loadView('expenses-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
