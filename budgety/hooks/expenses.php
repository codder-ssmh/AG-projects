...
function expenses_after_insert($data, $memberInfo, &$args) {
    ...
    // Populate Total expense of Related Client
    $currentTotalExpense = getCurrentExpenseOfRelatedClient($clientID);
    // Update Related Client Total expense
    // Total expense is Sum of Current expense and Newly added amount
    $updatedValue = $currentTotalExpense + $data['amount'];
    return updateTotalExpenseOfRelatedClient($clientID, $updatedValue);
}
...
function expenses_before_update(&$data, $memberInfo, &$args) {
    ...
    // Get Total expense of Related Client
    $currentTotalExpenseOfPrevClient = getCurrentExpenseOfRelatedClient($previousClientID);

    //If ammount is changed and Client is same
    if (($previousAmmount != $data['amount']) && ($previousClientID == $clientID)) {
        // Update Related Client Total expense
        // First Subtract previous ammount from Current Total expense then
        // Total expense is Sum of Subtracted Current expense and Newly updated amount
        $updatedValue = ($currentTotalExpenseOfPrevClient - $previousAmmount) + $data['amount'];
        return updateTotalExpenseOfRelatedClient($clientID, $updatedValue);
    } elseif ($previousClientID != $clientID) {
        //Client is changed
        // Remove Ammount from previous Client
        $updatedValue = $currentTotalExpenseOfPrevClient - $previousAmmount;
        updateTotalExpenseOfRelatedClient($previousClientID, $updatedValue);

        // Add Ammount to updated Client
        $currentTotalExpenseOfUpdatedClient = getCurrentExpenseOfRelatedClient($clientID);
        $updatedValue = $currentTotalExpenseOfUpdatedClient + $data['amount'];
        return updateTotalExpenseOfRelatedClient($clientID, $updatedValue);
    } else {
        return TRUE;
    }
...
function getClientIDOfRecord($recordID) {
    return sqlValue("select client from expenses where expense_id = '$recordID'");
}

//Get previous ammount of Selected Record
function getPreviousAmmountOfSelectedRecord($recordID) {
    return sqlValue("select amount from expenses where expense_id = '$recordID'");
}

//Get current expense of Related Client 
function getCurrentExpenseOfRelatedClient($clientId) {
    return sqlValue("select total_expense from clients where client_id = '$clientId'");
}

//UPDATE Related Client Total expense
function updateTotalExpenseOfRelatedClient($clientId, $updatedValue) {
...
function getUpdatedBudgetValue ($clientId, $updatedExpenseValue) {
    $totalIncome = sqlValue("select total_income from clients where client_id = '$clientId'");
    return $totalIncome - $updatedExpenseValue;
}
/**
 ...
 * An associative array where the keys are field names and the values are the field data values.
 * For this table, the array items are: 
 *     $data['expense_id'], $data['date'], $data['client'], $data['amount']
...
function expenses_before_delete($selectedID, &$skipChecks, $memberInfo, &$args) {
    ...
    $currentTotalExpense = getCurrentExpenseOfRelatedClient($clientID);
    //update Related Client Total expense
    //Total expense is  Current Total expense - Ammount of Record to delete
    $updatedValue = $currentTotalExpense - $previousAmmount;
    return updateTotalExpenseOfRelatedClient($clientID, $updatedValue);
}
