...
/**
 * @file
 * This file contains hook functions that get called when data operations are performed on 'incomes' table. 
 ...
 */
function incomes_init(&$options, $memberInfo, &$args) {
 ...
 */
function incomes_header($contentType, $memberInfo, &$args) {
    $header = '';
 ...
 */
function incomes_footer($contentType, $memberInfo, &$args) {
    $footer = '';
 ...
 *     $data['date'], $data['client'], $data['amount']
 ...
 */
function incomes_before_insert(&$data, $memberInfo, &$args) {
 ...
 */
function incomes_after_insert($data, $memberInfo, &$args) {
    // Escape non-numeric lookup values before using them in SQL queries 
    $clientID = makeSafe($data['client'], false);
    // Populate Total income of Related Client
    $currentTotalIncome = getCurrentIncomeOfRelatedClient($clientID);
    // Update Related Client Total income
    // Total income is Sum of Current income and Newly added amount
    $updatedValue = $currentTotalIncome + $data['amount'];
    return updateTotalIncomeOfRelatedClient($clientID, $updatedValue);
}
 ...
 */
function incomes_before_update(&$data, $memberInfo, &$args) {
    // Get the ammount of the record
    $recordID = makeSafe($data['selectedID'], false);
    // Get previous Ammount and CLient ID 
    $previousAmmount = getPreviousAmmountOfSelectedRecord($recordID);
    $previousClientID = getClientIDOfRecord($recordID);

    // Escape non-numeric lookup values before using them in SQL queries 
    $clientID = makeSafe($data['client'], false);

    // Get Total income of Related Client
    $currentTotalIncomeOFPrevClient = getCurrentIncomeOfRelatedClient($previousClientID);

    //If ammount is changed and Client is same
    if (($previousAmmount != $data['amount']) && ($previousClientID == $clientID)) {
        // Update Related Client Total income
        // First Subtract previous ammount from Current Total income then
        // Total income is Sum of Subtracted Current income and Newly updated amount
        $updatedValue = ($currentTotalIncomeOFPrevClient - $previousAmmount) + $data['amount'];
        return updateTotalIncomeOfRelatedClient($clientID, $updatedValue);
    } elseif ($previousClientID != $clientID) {
        //Client is changed
        // Remove Ammount from previous Client
        $updatedValue = $currentTotalIncomeOFPrevClient - $previousAmmount;
        updateTotalIncomeOfRelatedClient($previousClientID, $updatedValue);

        // Add Ammount to updated Client
        $currentTotalIncomeOFUpdatedClient = getCurrentIncomeOfRelatedClient($clientID);
        $updatedValue = $currentTotalIncomeOFUpdatedClient + $data['amount'];
        return updateTotalIncomeOfRelatedClient($clientID, $updatedValue);
    } else {
        return TRUE;
    }
...
//Get Client ID of Record
function getClientIDOfRecord($recordID) {
    return sqlValue("select client from incomes where income_id = '$recordID'");
}

//Get previous ammount of Selected Record
function getPreviousAmmountOfSelectedRecord($recordID) {
    return sqlValue("select amount from incomes where income_id = '$recordID'");
}

//Get current income of Related Client 
function getCurrentIncomeOfRelatedClient($clientId) {
    return sqlValue("select total_income from clients where client_id = '$clientId'");
}

//UPDATE Related Client Total income
function updateTotalIncomeOfRelatedClient($clientId, $updatedValue) {
    //Whenever update Income, Update Available Budget Too
    $updatedBudget = getUpdatedBudgetValue($clientId, $updatedValue);
    // Parameters to send to Update Function
    // Table name
    $tn = 'clients';
    // Assoc array of field names => values to be updated
    $set_array = array(
        'total_income' => $updatedValue,
        'available_budget' => $updatedBudget,
    );
    // Assoc array of field names => values used to build the WHERE clause
    $where_array = array('client_id' => $clientId);
    return update($tn, $set_array, $where_array);
}

//get Updated Available budget value
function getUpdatedBudgetValue($clientId, $updatedIncomeValue) {
    $totalExpense = sqlValue("select total_expense from clients where client_id = '$clientId'");
    return $updatedIncomeValue - $totalExpense;
}
 ...
 * An associative array where the keys are field names and the values are the field data values.
 * For this table, the array items are: 
 *     $data['income_id'], $data['date'], $data['client'], $data['amount']
 ...
 */
function incomes_after_update($data, $memberInfo, &$args) {
    return TRUE;
}
 ...
 */
function incomes_before_delete($selectedID, &$skipChecks, $memberInfo, &$args) {
    //get the ammount of the record
    $previousAmmount = getPreviousAmmountOfSelectedRecord($selectedID);
    //get Client ID 
    $clientID = getClientIDOfRecord(makeSafe($selectedID, false));
    // Escape non-numeric lookup values before using them in SQL queries 
    $clientID = makeSafe($clientID, false);
    //populate Total income of Related Client
    $currentTotalIncome = getCurrentIncomeOfRelatedClient($clientID);
    //update Related Client Total income
    //Total income is  Current Total income - Ammount of Record to delete
    $updatedValue = $currentTotalIncome - $previousAmmount;
    return updateTotalIncomeOfRelatedClient($clientID, $updatedValue);
}
 ...
 * None.
 */
function incomes_after_delete($selectedID, $memberInfo, &$args) {
 ...
 * None.
 */
function incomes_dv($selectedID, $memberInfo, &$html, &$args) {
 ...
 */
function incomes_csv($query, $memberInfo, &$args) {
 ...
 *   )
 */
function incomes_batch_actions(&$args) {
